...
async function isWinner(str) {
  try {
    let check1 = await db.getWinner(str)
    //console.log(check1)
    let id = check1.id
    if(check1.continent != "Europe") {
     return (`${str} is not what we are looking for because of the continent`)
    }
    let check2 = await db.getResults(id)
    let year = ""
    let result = ""
    //console.log(check2);
    let b = 0
    for (let items of check2) {
      year += String(items.year)
      result += items.score
      if (b < check2.length-1) {
        year += ", "
        result += ", "
        b++
      }
    ...
    return (`${str} won the FIFA World Cup in ${year} winning by ${result}`)
